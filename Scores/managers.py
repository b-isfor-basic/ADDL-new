...
from django.db.models.lookups import LessThanOrEqual, GreaterThan
...
class PlayerScoreSummaryManager(models.Manager):
    def get_queryset(self, *args, **kwargs):
        ...
        return (
            ...
            .annotate(
                ...
                avg_stars_per_game=(
                    Sum("total_stars", default=0) / (Count("id", distinct=True) * 10.0)
                ),
                # '01 stats
                ...
                rating=Case(
                    ...
                    When(LessThanOrEqual(F("rating_score"), 12.20), then=Value("D")),
                    When(LessThanOrEqual(F("rating_score"), 15.40), then=Value("C")),
                    When(LessThanOrEqual(F("rating_score"), 18.80), then=Value("B")),
                    When(LessThanOrEqual(F("rating_score"), 22.50), then=Value("A")),
                    When(GreaterThan(F("rating_score"), 22.50), then=Value("AA")),
                ),
            )
        )
    ...
    def get_player_stats_by_season_and_division(self, season_id, division_id):
        return self.get_queryset().filter(
            team__division__season_id=season_id, team__division_id=division_id
        )
