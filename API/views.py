...
def sub_list(request, season):
    if request.method == "GET":
        objects = Player.objects.all().prefetch_related('scoresummary_set', 'scoresummary_set__match', 'scoresummary_set__match__week')
        objects = objects.filter(scoresummary__match__week__season=season).filter(scoresummary__is_sub=True)
